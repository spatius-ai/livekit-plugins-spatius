...
from spatius import AudioFormat, LiveKitEgressConfig, OggOpusEncoderConfig, new_avatar_session
from spatius import AvatarSession as SpatiusSDKSession
...
class AvatarSession(BaseAvatarSession):
    ...
    def __init__(
        ...
        *,
        ...
        resolved_avatar_id = avatar_id if utils.is_given(avatar_id) else os.getenv("SPATIUS_AVATAR_ID")
        ...
        self._region = str(region if utils.is_given(region) else os.getenv("SPATIUS_REGION", DEFAULT_REGION))
        self._console_endpoint_url = str(
            console_endpoint_url if utils.is_given(console_endpoint_url) else os.getenv("SPATIUS_CONSOLE_ENDPOINT", "")
        )
        self._ingress_endpoint_url = str(
            ingress_endpoint_url if utils.is_given(ingress_endpoint_url) else os.getenv("SPATIUS_INGRESS_ENDPOINT", "")
        )
    ...
    async def start(
        ...
        *,
        ...
        if resolved_sample_rate <= 0:
            raise SpatiusException("sample_rate must be greater than 0")
        if self._audio_format == AudioFormat.OGG_OPUS and resolved_sample_rate not in SUPPORTED_OPUS_SAMPLE_RATES:
            raise SpatiusException(
    ...
    def _associate_request(self, segment: _SegmentState, req_id: str) -> None:
        ...
        if existing_segment is not None and existing_segment is not segment:
            logger.warning("Spatius request ID reused across audio segments", extra={"request_id": req_id})
    ...
    async def _wait_for_segment_completion_timeout(self, segment: _SegmentState, timeout: float) -> None:
        try:
    ...
    def _complete_segment(self, *, segment: _SegmentState, interrupted: bool, reason: str) -> bool:
        ...
        self._pending_segments = deque(candidate for candidate in self._pending_segments if candidate is not segment)
